...
# هنا غيرت البريفكس لـ "." مؤقتاً عشان ما يتصادم مع أي بوت ثاني
...
LEVEL_ROLES = {1: 1483227597801521316, 4: 1483227645016674384, 6: 1483227734728773784, 8: 1483227747563081841, 9: 1483227757981859950, 12: 1483227765669888092, 14: 1483227776638255307, 16: 1483227784737460225, 20: 1483227826718249031}
...
async def on_message(msg):
    if msg.channel.id == HOST_CH and not msg.author.bot:
        ...
        with open(DATA_FILE, "w") as f: json.dump(user_data, f, indent=4)
        if pls: await msg.add_reaction("⭐")
    await bot.process_commands(msg)

# هذي الأوامر الحين بتبدأ بنقطة (.) عشان ما تسوي كراش
@bot.command()
async def rank(ctx, m: discord.Member = None):
    m = m or ctx.author
    s = user_data.get(str(m.id), 0)
    e = discord.Embed(title="🎮 Toon Profile", color=0xffc0cb)
    e.add_field(name="الاسم", value=m.mention, inline=False)
...
async def top(ctx):
    ...
    await ctx.send(embed=discord.Embed(title="🏆 Top", description=txt or "None", color=0xffc0cb))
